...
class Pipeline:
    ...
    def video_postprocess(self, output: Union[torch.Tensor, np.ndarray]) -> av.VideoFrame:
        ...
        """
        # Convert to numpy and ensure RGBA format (4 channels)
        numpy_output = (output * 255.0).clamp(0, 255).to(dtype=torch.uint8).squeeze(0).cpu().numpy()

        # If we have 3 channels (RGB), convert to RGBA
        if numpy_output.shape[-1] == 3:
            # Create alpha channel (fully opaque)
            alpha = np.full((numpy_output.shape[0], numpy_output.shape[1], 1), 255, dtype=np.uint8)
            numpy_output = np.concatenate([numpy_output, alpha], axis=-1)

        return av.VideoFrame.from_ndarray(numpy_output, format='rgba')
